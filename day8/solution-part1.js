...
const instructionString = `kd dec -37 if gm <= 9
x dec -715 if kjn == 0
ey inc 249 if x < 722
n dec 970 if t > 3
f dec -385 if msg > -3
...
const findMaxValueInRegistry = () => {
  ...
  instructionSet.forEach(instruction => {
    ...
    let regToWriteIndex = registrySet.findIndex(
      registry => registry.label === instructionElements[0]
    );
    if (regToWriteIndex === -1) {
      registrySet.push({ label: instructionElements[0], value: 0 });
      regToWriteIndex = registrySet.length - 1;
    }
    // now I need to verify the condition

    let regToReadIndex = registrySet.findIndex(
      registry => registry.label === instructionElements[4]
    );
    ...
    const string = `${registrySet[regToReadIndex].value} ${
      instructionElements[5]
    } ${instructionElements[6]}`;

    const condition = eval(string);

    if (condition) {
      if (instructionElements[1] === 'inc') {
        registrySet[regToWriteIndex].value += parseInt(instructionElements[2]);
      } else {
        registrySet[regToWriteIndex].value -= parseInt(instructionElements[2]);
      }
    }
